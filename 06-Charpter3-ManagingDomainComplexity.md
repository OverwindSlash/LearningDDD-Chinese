...
正如地铁地图对航海导航毫无用处一样，在一个限界上下文中的通用语言可能与另一个限界上下文中的范围完全无关。限界上下文定义了通用语言和它所代表的模型的适用性范围。它允许根据不同的问题域定义不同的模型。换句话说，限界上下文是通用语言的一致性边界。通用语言中的术语、原则和业务规则只在其限界上下文中才是一致的。
...
这只是一块硬纸板吗？不，它是一个模型。这是西门子 KG86NAI31L 冰箱的模型。如果你上网查过，你可能会说这块硬纸板看起来一点也不像那个冰箱。它没有门，甚至它的颜色也不一样。
