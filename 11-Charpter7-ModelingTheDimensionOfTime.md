...
将数据同时写入操作数据库和日志文件是一个容易出错的操作。从本质上讲，它是针对两种存储机制的事务：数据库和文件。如果第一个失败，则第二个必须回滚。例如，如果一个数据库事务失败，没有人会关心删除之前的日志消息。因此，这样的日志不具有强一致性，也不具备最终一致性。
...
这种模式适合于对系统数据有深入了解是至关重要的场景，无论是为了分析和优化，还是因为法律要求的审计日志。
