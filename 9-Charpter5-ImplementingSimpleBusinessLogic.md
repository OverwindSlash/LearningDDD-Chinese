...
该模式的目标是封装将内存中的对象映射到数据库模式（schema）中的复杂性。除了负责持久化，活动记录对象还可以包含业务逻辑；例如，验证分配给字段的新值，或者甚至实现操作对象数据的与业务相关的过程。也就是说，活动记录对象的显著特征是数据结构和行为（业务逻辑）的分离。通常情况下，活动记录的字段有公共的 getters 和 setters，允许外部程序修改其状态。
